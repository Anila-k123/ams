...
function buildNodes(text, onClick, renderPlain) {
  const nodes = [];
  let last = 0;
  let m;
  const plain = (seg, key) => nodes.push(<Fragment key={key}>{renderPlain(seg)}</Fragment>);
  REF_RE.lastIndex = 0;
...
export default function CodeRefText({ text, className, renderPlain = (t) => t }) {
  const [pop, setPop] = useState(null);
  const token = localStorage.getItem("token");

  const lookup = async (act, section, e) => {
...
function CodeRefPopover({ pop, onClose }) {
  useEffect(() => {
  ...
  return createPortal(
    <>
      ...
      <div className="cr-pop" style={{ left, top }} onClick={(e) => e.stopPropagation()}>
        <button className="cr-pop-close" onClick={onClose}><FiX size={14} /></button>
