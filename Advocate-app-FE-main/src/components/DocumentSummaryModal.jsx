...
import { FiZap, FiRefreshCw, FiAlertTriangle, FiClock, FiDownload } from "react-icons/fi";
...
const RichText = ({ text }) => (
  <CodeRefText text={String(text)} renderPlain={(t) => <DefinableText text={t} />} />
);
...
function Section({ section, value }) {
...
export default function DocumentSummaryModal({ doc, onClose, canRegenerate = false }) {
  const [data, setData] = useState(null); // { status, summary, keyPoints, error, updatedAt }
  ...
  const downloadPdf = () => {
    ...
    const write = (text, { size = 11, bold = false, gap = 4, color = [30, 30, 30] } = {}) => {
  ...
  return (
    <Modal
      isOpen={!!doc}
      onClose={onClose}
      title={`Summary — ${doc?.documentName || "Document"}`}
    >
      <div className="ds-modal">
        {loading && (
          <div className="ds-state">
            <span className="ds-spinner" /> Loading summary…
          </div>
        )}
        ...
        {!loading && loadError && (
          <div className="ds-state ds-state-error">
            ...
            <button className="ds-btn" onClick={load}><FiRefreshCw /> Retry</button>
          </div>
        )}

        {!loading && !loadError && (status === "PENDING" || status === "PROCESSING") && (
          <div className="ds-state">
            <FiClock /> The summary is being generated. This usually takes a few seconds…
            <button className="ds-btn" onClick={load} disabled={busy}><FiRefreshCw /> Refresh</button>
          </div>
        )}

        {!loading && !loadError && status === "UNSUPPORTED" && (
          <div className="ds-state">
            <FiAlertTriangle /> A summary isn't available for this file type. Only
        ...
        {!loading && !loadError && (status === "NONE" || status === "FAILED") && (
          <div className="ds-state">
            {status === "FAILED" ? (
              <><FiAlertTriangle /> Summarization failed{data?.error ? `: ${data.error}` : "."}</>
            ) : (
              <><FiZap /> No summary has been generated for this document yet.</>
            )}
            {canRegenerate && (
              <button className="ds-btn ds-btn-primary" onClick={regenerate} disabled={busy}>
                <FiZap /> {busy ? "Starting…" : status === "FAILED" ? "Retry" : "Generate summary"}
              </button>
            )}
          </div>
        )}
        ...
        {!loading && !loadError && status === "READY" && (
          <div className="ds-content">
            {keyPoints.document_type && (
              <span className="ds-doctype">{keyPoints.document_type}</span>
            )}
            ...
            <div className="ds-footer">
              <button className="ds-btn ds-btn-primary" onClick={downloadPdf}>
                <FiDownload /> Download PDF
              </button>
              {canRegenerate && (
                <button className="ds-btn" onClick={regenerate} disabled={busy}>
                  <FiRefreshCw /> {busy ? "Regenerating…" : "Regenerate"}
                </button>
              )}
            </div>
          </div>
