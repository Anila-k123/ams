...
export async function fetchCourtDocument({ courtComplex, viewToken, kind, token, label }: any) {
...
// Throws Error (with .status) on failure.
export async function fetchHcBusiness(business: any) {
...
// Throws Error (with .status) on failure.
export async function downloadHcOrderPdf(url: string, label?: string) {
