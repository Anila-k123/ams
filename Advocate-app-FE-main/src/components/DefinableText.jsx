 ...
 */
export default function DefinableText({ text, className }) {
  const [pop, setPop] = useState(null); // { x, y, loading?, word, term?, definition?, notFound? }
  const token = localStorage.getItem("token");
  const authHeaders = { headers: { Authorization: `Bearer ${token}` } };

  const lookup = async (raw, e) => {
    ...
    try {
      const s = await axios.get(`/api/dictionary/search?q=${encodeURIComponent(q)}&limit=1`, authHeaders);
      if (!s.data || s.data.length === 0) { setPop({ ...anchor, word: q, notFound: true }); return; }
      const d = await axios.get(`/api/dictionary/term/${s.data[0].id}`, authHeaders);
...
function DefinePopover({ pop, onClose }) {
  useEffect(() => {
  ...
  return createPortal(
    <>
      ...
      <div className="def-pop" style={{ left, top }} onClick={(e) => e.stopPropagation()}>
        <button className="def-pop-close" onClick={onClose}><FiX size={14} /></button>
